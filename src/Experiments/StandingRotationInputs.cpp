#include "StandingRotationInputs.hpp"
...
StandingRotationInputs::StandingRotationInputs() : Experiment("StandingRotationInputs") {
  ...
  { Fitness("No movements",
            ...
            [](const Phenotype& p, float current, float dt) -> float {
              ...
              mmm::vec3 deltaPos = mmm::vec3(
                  p.initialPosition.x - massPos.x(),
                  p.initialPosition.y - massPos.y(),
                  p.initialPosition.z - massPos.z());

              return ExpUtil::score(dt, deltaPos.x) *
                     ExpUtil::score(dt, deltaPos.y) *
                     ExpUtil::score(dt, deltaPos.z) * current;
            }),
    Fitness("Time Alive",
  ...
  // ESHyperNEAT:
  // When a new connection, it will not be added if the
  // weight*maxWeightAndBias is less than 0.2
  //
  // HyperNEAT: Just used as a scale
  mSubstrate->m_max_weight_and_bias = 1.0;
...
StandingRotationInputs::~StandingRotationInputs() {
  delete mPopulation;
  delete mSubstrate;
}

void StandingRotationInputs::initPhenotype(Phenotype& p) const {
  p.fitness = mmm::vec<9>(1);
}

float StandingRotationInputs::mergeFitnessValues(const mmm::vec<9>& fitness) const {
  return mmm::product(fitness);
}

void StandingRotationInputs::outputs(Phenotype&                 p,
...
std::vector<double> StandingRotationInputs::inputs(const Phenotype& p) const {
