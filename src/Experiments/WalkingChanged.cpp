...
WalkingChanged::WalkingChanged() : Experiment("WalkingChanged") {
...
WalkingChanged::~WalkingChanged() {
  delete mPopulation;
  delete mSubstrate;
}

float WalkingChanged::mergeFitnessValues(const mmm::vec<9>& fitness) const {
  return fitness.x * fitness.y;
}

void WalkingChanged::outputs(Phenotype&                 p,
...
std::vector<double> WalkingChanged::inputs(const Phenotype& p) const {
