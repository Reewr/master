...
WalkingRTNK2::WalkingRTNK2() : Experiment("WalkingRTNK2") {
...

  {
    ...
    // is less than 0.2
    mSubstrate->m_max_weight_and_bias = 2.0;
...
WalkingRTNK2::~WalkingRTNK2() {
  delete mPopulation;
  delete mSubstrate;
}

float WalkingRTNK2::mergeFitnessValues(const mmm::vec<9>& f) const {
  return mmm::max(f.x - f.y, 0.f) * f.z * ExpUtil::score(1.f, f.w, 0.f);
}

void WalkingRTNK2::outputs(Phenotype&                 p,
...
std::vector<double> WalkingRTNK2::inputs(const Phenotype& p) const {
