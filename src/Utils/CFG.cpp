  ...
  parse = [&ab](const Prop& p, const Params& ps) {
    try {
      ...
      for(const auto& kv: Input::keyMap)
        valid_params += " " + kv.first;
      unknown_parameter(p, ps[0], valid_params);
    }
    ...
    try {
      ...
      for(const auto& kv: Input::keyMap)
        valid_params += " " + kv.first;
      unknown_parameter(p, ps[1], valid_params);
    }
  };
...
void remove_duplicates(StrCFG& strcfg, const std::vector<CFG::Prop>& duplicates) {
  size_t pos;

  for (const auto& prop: duplicates) {
    pos = prop.find(".");
...
std::ostream& operator<< (std::ostream& os, const CFG& cfg) {
  ...
  remove_duplicates(strcfg, cfg.duplicates);
