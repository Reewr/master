...
constexpr vec4::vec4 (const vec2& u, const vec2 t) : v{u[0],u[1],t[0],t[1]} {}
...
constexpr vec4 step(const vec4& v, const vec4& u) {
  return vec4(step(v[0], u[0]), step(v[1], u[1]), step(v[2], u[2]), step(v[3], u[3]));
}
...
constexpr vec4 max(const vec4& v, const vec4& u){
  return vec4(max(v[0], u[0]), max(v[1], u[1]), max(v[2], u[2]), max(v[3], u[3]));
}
...
constexpr vec4 min(const vec4& v, const vec4& u){
  return vec4(min(v[0], u[0]), min(v[1], u[1]), min(v[2], u[2]),  min(v[3], u[3]));
}
