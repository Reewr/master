...
void GL::Grid3D::generateNormals(const dVector<vec3>& v, dVector<vec3>& n) {
  ...
  for(int y = 0; y < size.y-1; y++) {
    for(int x = 0; x < size.x-1; x++) {
      vec3 triangleNorm0 = cross(v[y][x]-v[y+1][x], v[y+1][x]-v[y+1][x+1]);
      vec3 triangleNorm1 = cross(v[y+1][x+1]-v[y][x+1], v[y][x+1]-v[y][x]);
...
void GL::Grid3D::setupOpenGLArrays(const vector<Vertex>& v, const vector<int>& i) {
  glGenBuffers(1, &VBO);
  glGenBuffers(1, &IBO);
