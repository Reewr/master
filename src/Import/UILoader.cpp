...
bool Import::UILoader::loadXMLSettings(std::string filepath, std::string settings, Window* win) {
  if(!loadXML(filepath))
    return false;
...
void Import::UILoader::getAttrib(tinyxml2::XMLElement* e, const char* a, int* t) {
...
void Import::UILoader::getAttrib(tinyxml2::XMLElement* e, const char* a, float* t) {
...
void Import::UILoader::getAttrib(tinyxml2::XMLElement* e, const char* a, const char* b, vec2& v) {
  getAttrib(e, a, &v.x);
  getAttrib(e, b, &v.y);
}

std::string Import::UILoader::getAttrib(tinyxml2::XMLElement* e, const char* a) {
...
void Import::UILoader::handleWindowElement(tinyxml2::XMLElement* winElem, Window* win, bool isOnlyWin) {
...
void Import::UILoader::handleMenuElement(tinyxml2::XMLElement* elem, Window* w) {
  tinyxml2::XMLElement* itemElem = elem->FirstChildElement();
  for(; itemElem != NULL; itemElem=itemElem->NextSiblingElement()) {
    Menu::MenuSettings ms;
    vec2 pos;
    std::string mName = std::string(itemElem->Parent()->ToElement()->Attribute("name"));
...
void Import::UILoader::handleMenuItemElement(tinyxml2::XMLElement* elem, Window* w) {
  tinyxml2::XMLElement* i = elem->FirstChildElement();
  for(; i != NULL; i=i->NextSiblingElement()) {
    if (i->Name() == std::string("guidropdown")) handleDropdownElement(i, w);
    else if (i->Name() == std::string("guicheckbox")) handleCheckboxElement(i, w);
    else if (i->Name() == std::string("guiinputbox")) handleInputboxElement(i, w);
    else if (i->Name() == std::string("guislider"))   handleSliderElement(i, w);
  }
}

void Import::UILoader::handleDropdownElement(tinyxml2::XMLElement* e, Window* w) {
  vec2 pos;
...
void Import::UILoader::handleCheckboxElement(tinyxml2::XMLElement* e, Window* w) {
  std::string name = std::string(e->Attribute("name"));
...
void Import::UILoader::handleInputboxElement(tinyxml2::XMLElement* e, Window* w) {
  std::string name = std::string(e->Attribute("name"));
...
std::string Import::UILoader::getError(int error) {
  switch(error) {
    case FILEERROR: return "Failed at loading UI XML file: ";
    case LOADSETTERROR: return "Tried accessing invalid XML file: ";
    case INVSCREEN: return "Invalid setting given to UILoader::loadSetting: ";
    case ATTRIBINTFAIL: return "QueryIntAttribute at Element: ";
    case ATTRIBFLOFAIL: return "QueryFloatAttribute at Element: ";
    case ATTRIBSTRFAIL: return "Atttribute at Element: ";
  }
