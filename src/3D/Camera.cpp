...
mat4 Camera::updateViewMatrix () {
  vec3 cp = target + vec3 (rotate_y(hrot) * rotate_x(vrot) * vec4 (0, 0, height, 1));
  vec3 cu = vec3 (rotate_y(hrot) * rotate_x(vrot) * vec4 (0, 1, 0, 0));
  return lookAt (cp, target, cu);
}

mat4 Camera::updateProjMatrix () {
  return perspective (67, asset->cfg.graphics.aspect, 0.1, asset->cfg.graphics.viewDistance);
}
...
void Camera::update (float) {
  ...
  //float h = height;
  //light.mvp.proj = ortho (-5*height, 5*height, -5*height, 5*height, -5*height, 5*height);
...
void Camera::handleKeys (std::vector<int> actions, float dt) {
  ...
  for (unsigned int i = 0; i < actions.size(); i++) {
    if (actions[i] == Input::MOVE_UP)   target += forward;
    if (actions[i] == Input::MOVE_DOWN) target -= forward;

    if (actions[i] == Input::MOVE_LEFT)  target += strafe;
    if (actions[i] == Input::MOVE_RIGHT) target -= strafe;

    if (actions[i] == Input::ROTATE) {
      vec2 press = input->getPressedCoord(input->getKey(Input::ROTATE).key1);
      vec2 curr = input->getMouseCoords();

      float rsh = asset->cfg.camera.rotSpeed * (asset->cfg.camera.rotInvH?-1:1);
      float rsv = asset->cfg.camera.rotSpeed * (asset->cfg.camera.rotInvV?-1:1);
