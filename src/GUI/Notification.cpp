...
Notification::Notification(const Rect& rect,
  ...
  mButtonText->setPosition(vec2(
    mBoundingBox.middle().x - mButtonText->box().bottomright().x/2,
    mBoundingBox.bottomright().y - 20)
  );
  mIconRect = Rect(
    vec2(mBoundingBox.bottomright().x - 50, mBoundingBox.topleft.y + 12.5),
    vec2(50, 50)
  );
...
void Notification::addNewNotification(const int reason) {
  switch(reason) {
    ...
    case Notification::IDLEROBOT:
      mText->setText("Queues are building up at the charging stations. I suggest building more of them.");
    break;
  }
}
...
void Notification::runAnimation(float deltaTime) {
  ...
  vec2 movement(
    (mNewPos.x/mAnimationTime) * deltaTime,
